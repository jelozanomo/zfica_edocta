sap.ui.define([
	...
	return BaseObject.extend("ean.edu.co.edo.cta.model.GroupSortState", {
		 ...
		 * @alias ean.edu.co.edo.cta.model.GroupSortState
		 */
		...
		/**
		 * Groups by Betrh, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Betrh") {
				...
				this._oViewModel.setProperty("/sortBy", "Betrh");

				aSorters.push(
					new Sorter("Betrh", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "TxtList");
			}
